...
RED FLAG 7 — never asked whether info/interest was for the lead alone or included a spouse/family member: Every call should include some version of this standard opening question (e.g. "was that just for you, or another family member as well?"). If the rep never asked anything like this at any point in the call, set red_flag_spouse_never_asked to true. If they asked it in any reasonable form, set it to false.

RED FLAG 8 — spouse exists but attendance never confirmed: ONLY evaluate if the transcript establishes the lead has a spouse/partner (whether the prospect volunteered this or the rep asked and found out) AND an appointment was scheduled. If so, and the rep never specifically asked whether the spouse would be attending/joining the appointment, set red_flag_spouse_attendance_not_asked to true. If they did ask, or there's no indication of a spouse, or no appointment was scheduled, set it to false.

RED FLAG 9 — spouse exists but name never captured: ONLY evaluate if the transcript establishes the lead has a spouse/partner. If so, and the rep never obtained the spouse's name at any point in the call, set red_flag_spouse_name_not_captured to true. If they got the name, or there's no indication of a spouse, set it to false.
...
COACHING TIP: this must be SPECIFIC to what actually happened in THIS call, not generic advice. Reference the actual objection/scenario this persona raised and exactly what the rep could have said or asked differently at the actual moment it mattered (quote or closely paraphrase that moment). Then name ONE concrete skill to practice before their next attempt (e.g. "practice asking a specific follow-up question after someone mentions a spouse" rather than "ask better questions"). Avoid generic phrases like "build more rapport" or "listen better" without tying them to what specifically happened on this call.
