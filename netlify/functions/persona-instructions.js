// persona-instructions — the shared "brain" behind every persona, used by
// BOTH realtime-session.js (OpenAI) and gemini-session.js (Google). This is
// deliberately provider-agnostic: it only builds the instructions STRING.
// Each provider's own file handles its own session config (voice names,
// audio format, model name, etc.), which differ between the two.
//
// Keeping this in one place means a persona-behavior fix (pacing, realism,
// scheduling gate, etc.) applies identically to both engines automatically
// — no risk of the two providers' persona behavior drifting apart over time.
...
// "Already taken care of" claims (has-will / has-plans / has-plot objection
// type): most real families who say this only mean a basic will, a life
// insurance policy, or an existing cemetery plot — NOT an actual funeral
// home pre-need plan. A good rep has to clarify which one it is before
// treating the objection as resolved. Occasionally (rare, at your own
// discretion) it's genuinely the latter — and in that case scheduling
// really isn't appropriate, which is exactly the scenario the scoring
// rubric checks for as a red flag.
const HAS_PLANS_CLARIFICATION = `"ALREADY TAKEN CARE OF" CLAIMS (special instruction): When you say things are "already taken care of" or "we already have a plan," be deliberately vague about what that actually means at first — don't specify whether it's a will, a life insurance policy, an existing cemetery plot, or an actual pre-arranged plan with a funeral home. Make the rep ask a clarifying question (e.g. "when you say taken care of, do you mean you've already met with a specific funeral home and set up payments toward a pre-arranged plan, or is that more about a will, a life insurance policy, or a cemetery plot you already have?") before you clarify what you actually meant.

MOST OF THE TIME (the common, realistic case): once asked, reveal that it's really just a will, a life insurance policy, and/or an existing cemetery plot — none of which actually cover the funeral service itself, which is the distinction a good rep should draw out and explain. Once they do, you can be reasonably persuaded to still consider an appointment (subject to the scheduling gate above).
...
// Builds the full instructions string for a persona given the same
// parameter set both realtime-session.js and gemini-session.js collect
// from the frontend. Provider-specific wrapping (voice name, audio config,
// model choice) stays in each provider's own file.
function buildPersonaInstructions({ personaPrompt, fhName, fhPronunciation, afpName, leadSource, difficultyLevel, objectionType, address, surveyDateLabel, personaEmail }) {
  const backstory = LEAD_SOURCE_BACKSTORY[leadSource] || '';
  const difficultyDesc = describeDifficulty(difficultyLevel);
  const hasPlansNote = ['has-will', 'has-plans', 'has-plot'].includes(objectionType) ? HAS_PLANS_CLARIFICATION : '';
  const fhLine = fhName
  ...
  // output. Stripping it out at the source (rather than just contradicting
  // it later) removes any conflicting signal for the model.
  const cleanedPersonaPrompt = (personaPrompt || '')
    .replace(/\s*Respond only in the required JSON format\.?\s*$/i, '')
    .trim();

  return `${cleanedPersonaPrompt}
...
module.exports = { buildPersonaInstructions };
